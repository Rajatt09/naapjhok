...
import { MapPin, Phone, Calendar, User, AlertCircle, CheckCircle, ChevronLeft, ShoppingBag } from 'lucide-react';
...
const Booking = () => {
    ...
    const [targetProfileId, setTargetProfileId] = useState(() => {
        return location.state?.profileId || sessionStorage.getItem('bookingProfileId') || null;
    });
    ...
    const profileDetails = useMemo(() => {
        ...
        return profiles.find(p => p._id === targetProfileId || p.name === targetProfileId) || { name: 'Valued Customer', isSelf: false };
    }, [profiles, targetProfileId]);
    
    // ... items calculation
    const bookingItems = useMemo(() => {
        if (!targetProfileId) return [];
        return cart.filter(item => (item.profileId || 'me') === targetProfileId);
    ...
    const handleSubmit = async (e) => {
        ...
        const orderData = {
            items: bookingItems.map(item => ({
                product: item.product._id || item.product.id, // Ensure we send the Product ID
                quantity: 1, 
                withFabric: item.withFabric,
                price: item.withFabric ? (Number(item.product.basePrice) + Number(item.product.fabricPrice)) : Number(item.product.basePrice),
                customization: item.customization ? JSON.stringify(item.customization) : '' // Stringify if schema expects String
            })),
        ...
        try {
            ...
            alert(`Appointment Booked for ${profileDetails ? profileDetails.name : 'you'}!`);
            navigate('/profile');
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="min-h-screen bg-brand-cream pt-32 pb-20 px-4">
            <div className="container-custom max-w-6xl">
                ...
                {/* Back Link */}
                <button onClick={() => navigate('/cart')} className="flex items-center gap-2 text-brand-taupe hover:text-brand-coffee mb-8 transition-colors group text-sm uppercase tracking-wider font-medium">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
                    ...
                    <div className="lg:col-span-7 space-y-8 animate-in slide-in-from-left duration-500">
                        <div>
                            <h2 className="text-4xl font-serif text-brand-espresso mb-3 leading-tight">Secure Your <br/>Appointment</h2>
                            <p className="text-brand-taupe text-lg">Booking a master visit for <strong className="text-brand-coffee">{profileDetails ? profileDetails.name : 'Loading...'}</strong>.</p>
                        </div>

                        <form className="bg-white/50 p-6 md:p-10 rounded-2xl border border-brand-brown/5 space-y-10" onSubmit={handleSubmit}>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-brand-taupe uppercase tracking-wider">Name <span className="text-red-500">*</span></label>
                                        <input 
                                            type="text" name="name" 
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-brand-taupe uppercase tracking-wider">Phone <span className="text-red-500">*</span></label>
                                        <div className="relative">
                                            ...
                                            />
                                            <Phone className="absolute left-4 top-4 text-brand-taupe/50" size={18} />
                                        </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                     <label className="text-xs font-bold text-brand-taupe uppercase tracking-wider">Full Address <span className="text-red-500">*</span></label>
                                     <div className="relative">
                                         <textarea 
                                            name="address" rows="3" 
                            ...
                            <div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-brand-taupe uppercase tracking-wider">Date <span className="text-red-500">*</span></label>
                                        <input 
                                            type="date" name="date" 
                                        ...
                                        />
                                        {errors.date && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle size={10} /> {errors.date}</p>}
                                    </div>
                                    
                                    <div className="space-y-3">
                                        <label className="text-xs font-bold text-brand-taupe uppercase tracking-wider mb-2 block">Time Slot <span className="text-red-500">*</span></label>
                                        ...
                                        {errors.timeSlot && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle size={10} /> {errors.timeSlot}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-brand-brown/10">
                                <Button 
                                    ...
                                    disabled={loading}
                                >
                                    {loading ? 'Processing...' : `Confirm Appointment • ₹${bookingTotal}`}
                                </Button>
                                <p className="text-[10px] text-brand-taupe text-center mt-3">
                                    By clicking confirm, our master will be notified to visit your location.
                                </p>
                            </div>
                    ...
                    <div className="lg:col-span-5 animate-in slide-in-from-bottom duration-700 delay-100 hidden lg:block">
                        <div className="bg-white p-8 rounded-2xl shadow-xl sticky top-32 border border-brand-brown/10">
                            <h3 className="text-xl font-serif text-brand-espresso mb-6 flex items-center gap-2 pb-4 border-b border-brand-brown/10">
                                <ShoppingBag size={20} /> Order Summary <span className="text-sm font-sans text-brand-taupe ml-auto font-normal">({bookingItems.length} items)</span>
                            </h3>
                                    ...
                                    <div key={item._id} className="flex justify-between items-start gap-4">
                                       <div className="flex gap-4">
                                            <div className="w-12 h-16 rounded overflow-hidden bg-brand-cream shrink-0">
                                                 <img src={item.product?.image || item.image || '/placeholder.jpg'} alt={item.product?.name || item.name} className="w-full h-full object-cover opacity-90" />
                                            </div>
                                            <div>
                                                <h4 className="text-sm font-bold text-brand-espresso leading-tight">{item.product?.name || item.name}</h4>
                                                <p className="text-xs text-brand-taupe mt-1">{item.product?.category}</p>
                                                <p className="text-xs text-brand-taupe mb-4">{item.withFabric ? 'Bespoke Stitching & Fabric' : 'Stitching Service Only'}</p>
                                                {item.withFabric && <p className="text-[10px] text-brand-rust uppercase tracking-wider font-bold mt-1">+ Fabric</p>}
                                            </div>
                                       </div>
                                       <div className="text-sm font-medium text-brand-coffee whitespace-nowrap">
                                            ₹{item.withFabric ? (Number(item.product?.basePrice) + Number(item.product?.fabricPrice)) : Number(item.product?.basePrice)}
                                       </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="lg:hidden mt-8 border-t border-brand-brown/10 pt-8">
                        <h4 className="font-serif text-brand-espresso text-lg mb-4">Order Summary ({bookingItems.length} Items)</h4>
                          <div className="space-y-4">
                                {bookingItems.map(item => (
                                    <div key={item._id} className="flex items-center justify-between py-2 border-b border-brand-brown/5 last:border-0">
                                        <span className="text-sm text-brand-coffee">{item.product?.name || item.name}</span>
                                        <span className="text-sm font-bold text-brand-espresso">₹{item.withFabric ? (Number(item.product?.basePrice) + Number(item.product?.fabricPrice)) : Number(item.product?.basePrice)}</span>
                                    </div>
                                ))}
